...
class MovementSystem {
  ...
  public getReachablePositions(gameMap: GameMap): {
    ...
    const queue: { pos: Position; cost: number }[] = [{ pos: currentUnit.position, cost: 0 }];
    ...
    while (queue.length > 0) {
      ...
      this.getNeighbors(pos).forEach((neighbor) => {
        if (!this.isValidPosition(neighbor, width, height)) {
          return;
        }
    ...
    return {
      reachable,
      nearestEnemyPosition: minDistance < Infinity ? nearestEnemyPosition : currentUnit.position,
    };
  }
